...
#### ==Версия v5.05 от 2025-04-04==
- `[*]` Исправлен двойной реверс стрелки после переворота уровня ретеста
- `[*]` При включенном реверсе 'ARR.R' в режиме ретеста 'RT.E' положение уровень ретеста по 'RT.BR' переворачивается и меняется направление его пробития

#### Версия v5.03 от 2025-04-03
- `[+]` Режим сравнения плечей `LSF.SC.CM`: Базовый или инвертированный

#### Версия v5.02 от 2025-04-02
- `[+]` `LSF.SC.ATR`: Мультипликатор ATR для мин. дистанции между экстремумами плечей
- `[+]` `OSB.CNT`: Мин. кол. баров одного напр-я перед свечой обр.направления

#### Версия v5.01 от 2025-03-27
- `[*]` Исправлена ошибка построения Ретест-уровня более чем через одну свечу от пробития Фрактал-уровня

#### Версия v5.00 от 2025-03-27

Блок настроек `РЕТЕСТ (RT)` теперь работает и при отключенной дивергенции в параметре `DV.DIV.M`. Все настройки работают также. Дополнительно добавлен фильтр проверки положения свечи обратного направления, которую бот ждет после касания. Можно запретить формировать уровень ретеста по свече обратного направления, если ее Open или Close расположены хуже исходного уровня - `RT.OWL.V5` и `RT.СWL.V5` соответственно. 

![v5.00 Ретест](img/UM008.%20V5_Retest.png)

#### ==Версия v4.02 от 2025-03-15==
- `[*]` Расчет начала левого плеча и начало мониторинга пробития бот выполняет от свечи экстремума на ТФ графика. Было от свечи начала фрактала на ТФ `F.FTF`
![Экстремумы фракталов](img/UM007.v4.02.png)
...
- [x] `RT.OWL.V5`: Запрещен Open обратной свечи хуже уровня
- [x] `RT.CWL.V5`: Запрещен Сlose обратной свечи хуже уровня
...
- [x] `LSF.SC.ATR`: Мультипликатор ATR для мин. дистанции между экстремумами плечей
- [x] `LSF.SC.CM`: Режим сравнения вершин плечей
    - Базовый: `SUP(R>L) и RES(R<L)`
    - Инвертированный: `SUP(R<L) и RES(R>L)`
- [x] `OSB.CNT`: Мин. кол. баров одного напр-я перед свечой обр.направления
